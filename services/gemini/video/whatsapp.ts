...
class WhatsAppVideoGeneration {
  ...
  validateOperationResponse(operation: unknown): ValidationResult {
    ...
    if (!op.response) {
      logger.error('❌ No response in operation:', operation as Error);
      return {
        success: false,
    ...
    if (!op.response.generatedVideos) {
      logger.error('❌ No generatedVideos in response:', op.response as Error);
    ...
    if (!op.response.generatedVideos.length || !op.response.generatedVideos[0]) {
      logger.error('❌ Empty generatedVideos array:', op.response.generatedVideos as Error);
    ...
    if (!op.response.generatedVideos[0].video) {
      logger.error('❌ No video file in first generated video:', op.response.generatedVideos[0] as Error);
      return {
        success: false,
  ...
  async generateVideoFromImageForWhatsApp(prompt: string, imageBuffer: Buffer, req: Request | null = null): Promise<WhatsAppVideoResult> {
    try {
      logger.info('🎬 Starting Veo 3 image-to-video generation - Stable version');
      const cleanPrompt = sanitizeText(prompt);
      ...
      logger.info('✅ Veo 3 image-to-video generated and converted successfully');
      logger.info(`🎬 Video saved to: ${finalFilePath}`);
      logger.info(`🔗 Public URL: ${videoUrl}`);
