...
class TasksManager {
  ...
  private repository: TasksRepository | null;

  constructor(conversationManager: ConversationManager, repository: TasksRepository | null = null) {
  ...
  async saveTask(taskId: string, status: string, data: TaskData = {}): Promise<void> {
    ...
    // Fallback to legacy direct SQL if repository not provided (should not happen in new container)
